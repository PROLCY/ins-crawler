...
def read_image_pk_from_data_csv():
    ...
    with open('data.csv', "r") as csv_file:
        ...
        if(len(data_lines) == 0):
            return image_pk
        image_pk = int(data_lines[-1].split(',')[0].split('.')[0].split('_')[-1])+1
...
def download_photo(photo, image_pk, prev_pk_list, client):
    ...
    if(photo_pk in prev_pk_list):
        return None, None
    basename = 'i_' + str(image_pk)
    download_image_path = client.photo_download_by_url(photo_url, basename, folder=Path(IMAGE_DIR_PATH))

    print(download_image_path)

    ext = '.' + str(download_image_path).split('.')[-1]
    filename = basename + ext
    return photo_pk, filename

## data.csv에 data(image_pk, hashtag, 좋아요 수, 사진 올린 사람) 적는 함수
def write_data_in_data_csv(filename, hashtag, photo):
    csv_file = open('data.csv', 'a')
    csv_writer = csv.writer(csv_file)
    csv_writer.writerow([filename, hashtag, photo.like_count, photo.user.username])
    csv_file.close()
...
def download_photos(photos, hashtag, prev_pk_list, client):
    ...
    for photo in photos:
        ## 사진 다운로드
        photo_pk, filename = download_photo(photo, image_pk, prev_pk_list, client)
        if(photo_pk == None):
            continue

        recent_pk_list.append(photo_pk)
        write_data_in_data_csv(filename, hashtag, photo)
